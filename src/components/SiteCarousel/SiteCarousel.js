...
class SiteCarousel extends React.Component{
    ...
    constructor(props) {
      ...
      previous() {
        if (this.animating) return;
        const nextIndex = this.state.activeIndex === 0 ? this.props.vehiclesData.length - 1 : this.state.activeIndex - 1;
        this.setState({ activeIndex: nextIndex });
      }
        ...
        const slides = this.props.vehicleData.map((item) => {
          return (
            ...
            >
              ...
              <img src={item.carouselImage} className={"carouselImage"} alt={item.altText} /></a>
